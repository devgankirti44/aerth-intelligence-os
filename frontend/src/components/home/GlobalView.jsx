...
// Map tab -> domains that count for that view
const TAB_DOMAINS = {
  Overview: null, // all
  Geopolitics: ['geopolitics'],
  Economy: ['currency_macro', 'commodities'],
  Technology: ['india_manufacturing', 'company'],
  Markets: ['currency_macro', 'commodities'],
  Society: ['food_security', 'india_agriculture'],
  Environment: ['energy_transition', 'india_agriculture']
};

const TAB_NODE_TYPES = {
  Overview:    ['chart', 'users', 'gov', 'leaf', 'doc'],
  Geopolitics: ['gov', 'gov', 'shield', 'shield', 'gov'],
  Economy:     ['chart', 'chart', 'gold', 'chart', 'gold'],
  Technology:  ['chip', 'chip', 'users', 'chip', 'chart'],
  Markets:     ['chart', 'gold', 'chart', 'gold', 'chart'],
  Society:     ['users', 'users', 'leaf', 'users', 'doc'],
  Environment: ['leaf', 'leaf', 'leaf', 'doc', 'leaf']
};

export default function GlobalView() {
  const [activeTab, setActiveTab] = useState('Overview');
  const [signals, setSignals] = useState([]);
  const [risingIndex, setRisingIndex] = useState(0);

  useEffect(() => {
    axios.get(`${API}/world-intel/dashboard`)
      .then(res => setSignals(res.data.latestSignals || []))
      .catch(() => {});
  }, []);

  // Filter signals by active tab domain
  const filteredSignals = (() => {
    const domains = TAB_DOMAINS[activeTab];
    if (!domains) return signals;
    return signals.filter(s => domains.includes(s.domain));
  })();

  // Rotate rising signal every 6 sec
  useEffect(() => {
    if (filteredSignals.length < 2) return;
    const interval = setInterval(() => {
      setRisingIndex(i => (i + 1) % Math.min(filteredSignals.length, 4));
    }, 6000);
    return () => clearInterval(interval);
  }, [filteredSignals.length]);

  // Reset rising index when tab changes
  useEffect(() => {
    setRisingIndex(0);
  }, [activeTab]);

  const rising = filteredSignals[risingIndex] || filteredSignals[0];
  const totalDots = Math.min(filteredSignals.length, 4) || 1;
  ...
  return (
    <div className="globe-panel">
      <div className="globe-panel__header">
        <div className="globe-panel__title-block">
          ...
          <div className="globe-panel__live">
            ...
            <span>Live · {filteredSignals.length} signals</span>
          </div>
        </div>
      ...
      <div className="globe-panel__globe">
        <ThreeGlobe />
        <FloatingNodes tabName={activeTab} />
      </div>

      {rising ? (
        <a href={rising.url} target="_blank" rel="noreferrer" className="globe-panel__signal">
          ...
          <div className="globe-panel__signal-content">
            ...
            <h3 className="globe-panel__signal-title">{truncate(rising.title, 70)}</h3>
            <p className="globe-panel__signal-desc">{truncate(rising.summary || rising.source, 90)}</p>
            <div className="globe-panel__signal-meta">
              <span>{timeAgo(rising.publishedAt)}</span>
              <span className="globe-panel__signal-dot">•</span>
              <span>{(rising.domain || activeTab).replace(/_/g, ' ')}</span>
            </div>
          </div>
          <span className="globe-panel__signal-btn">→</span>
        </a>
      ) : (
        <Link to="/world-intelligence" className="globe-panel__signal">
          <div className="globe-panel__signal-icon"><TrendUpIcon /></div>
          <div className="globe-panel__signal-content">
            <span className="globe-panel__signal-label">— NO {activeTab.toUpperCase()} SIGNALS</span>
            <h3 className="globe-panel__signal-title">Switch category or view all in World Intelligence</h3>
          </div>
          <span className="globe-panel__signal-btn">→</span>
        </Link>
      )}
      ...
      <div className="globe-panel__dots">
        {Array.from({ length: totalDots }).map((_, i) => (
          <span 
            key={i} 
            className={`globe-panel__dot ${i === risingIndex ? 'globe-panel__dot--active' : ''}`} 
          />
        ))}
      </div>
    </div>
...
function ThreeGlobe() {
  ...
  useEffect(() => {
    ...
    const ringMaterial = new THREE.LineBasicMaterial({
      color: 0xCFC4A8, transparent: true, opacity: 0.35
    });
    const ringTilts = [
      { x: 0.3, y: 0, z: 0.2 },
      { x: -0.4, y: 0.3, z: -0.1 },
    ...
    const dotMaterial = new THREE.MeshBasicMaterial({
      color: 0xEAE6DC, transparent: true, opacity: 0.9
    });
    const dots = [];
...
/* Floating nodes — change based on active tab */
function FloatingNodes({ tabName }) {
  const nodeTypes = TAB_NODE_TYPES[tabName] || TAB_NODE_TYPES.Overview;
  const positions = [
    { left: '30%', top: '38%' },
    { left: '54%', top: '32%' },
    { left: '72%', top: '42%' },
  ...
  return (
    <>
      {nodeTypes.map((type, i) => (
        <div
          key={`${tabName}-${i}`}
          className="globe-viz__node globe-viz__node--fade"
          style={positions[i]}
        >
          <NodeBadge type={type} variant={i % 2 === 0 ? 'green' : ''} />
        </div>
      ))}
    </>
  );
...
function NodeIcon({ type }) {
  ...
  if (type === 'chip')   return <svg {...props}><rect x="6" y="6" width="12" height="12" rx="1"/><path d="M9 3v3M15 3v3M9 18v3M15 18v3M3 9h3M3 15h3M18 9h3M18 15h3"/></svg>;
  if (type === 'gold')   return <svg {...props}><circle cx="12" cy="12" r="7"/><path d="M12 8v8M9 12h6"/></svg>;
  return null;
}
